...
export const AppProvider = ({ children }: PropsWithChildren) => {
  ...
  const socket = useMemo(
    () =>
      io(`${process.env.NEXT_API_URL}/?token=${token}`, {
        autoConnect: false,
      }),
    [token]
  );
  ...
  const _addMessage = (
    ...
    setMessages((msgs) => {
      const h = [...msgs];
      const index = msgs.findIndex((el) => el._id === id && el.isMe === isMe);
      if (index > -1) {
  ...
  const verify: IContext["verify"] = useCallback(
    (code, email) => authApi.verify(code, email).then((r) => setUser(r.data)),
    []
  );
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .catch((e: AxiosError) => console.error("Error while authorizing", e));
  }, [token]);
